...
public static class Limit
{
    ...
    /// </remarks>
    /// <exception cref="ArgumentOutOfRangeException"><paramref name="permits"/> is less than one, <paramref name="queueLimit"/> is negative, or <paramref name="window"/> is not positive.</exception>
...
public static class RateLimiterExtensions
{
    ...
    /// <returns>The acquired lease.</returns>
    /// <exception cref="RateLimitedException">The limiter refused.</exception>
    public static ValueTask<RateLimitLease> AcquireOrThrowAsync<TKey>(this PartitionedRateLimiter<TKey> limiter, TKey key, CancellationToken cancellationToken = default)
        where TKey : notnull =>
    ...
    /// <returns>The acquired lease.</returns>
    /// <exception cref="RateLimitedException">The limiter refused.</exception>
    public static async ValueTask<RateLimitLease> AcquireOrThrowAsync<TKey>(this PartitionedRateLimiter<TKey> limiter, TKey key, string? name, CancellationToken cancellationToken = default)
        where TKey : notnull
    {
        ArgumentNullException.ThrowIfNull(limiter);
