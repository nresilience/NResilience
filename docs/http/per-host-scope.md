...
blast-radius inversion a breaker exists to prevent.
...
breaker is a deliberate scope decision and the switch does not overrule it. The same is true of an
...
> [!WARNING]
> The registry is unbounded on purpose: the set of hosts one `HttpClient` talks to is a property of the
