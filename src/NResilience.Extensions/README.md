...
Dependency injection, configuration binding, rate and concurrency limiting, and OpenTelemetry-shaped telemetry
for [NResilience](https://github.com/nresilience/NResilience).
...
- `AddResilience()` on `IHttpClientFactory` - the one line most apps need. Adds the resilience handler, stops `HttpClient.Timeout` competing with the policy
  deadline, names the policy after the client, and attaches the telemetry meter and log listener.
...
- Configuration binding with hot reload - bind a `Resilience` value from `IConfiguration`. A reload is a reference swap, not a rebuild, so breaker and budget
  state survive.
...
- [Dependency injection](https://github.com/nresilience/NResilience/blob/main/docs/di/index.md) - `AddResilience`, named policies, hot reload, state
  persistence.
