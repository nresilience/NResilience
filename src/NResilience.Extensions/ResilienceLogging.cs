...
public static class ResilienceLogging
{
    ...
    /// <param name="logger">Where the records go. Its category is what an <c>appsettings.json</c> filter matches, so <see cref="CategoryFor(string?)"/> is how to create it.</param>
    /// <param name="options">The profile, the repeat window and the level delegate. Null takes the defaults.</param>
