...
public static class ResilienceHttp
{
    ...
    /// amplification.
    /// </remarks>
    public static string NestedRetryHeader => "X-NResilience-Retrying";
        ...
        HttpMessageHandler? innerHandler = null)
    {
        ...
        {
            // HttpClient.Timeout applies to the whole send - retries, backoff and all - and it is
            // the one bound nothing in the policy can see. Two timeout systems interacting
            // silently is the worst available default, so the deadline on the policy becomes the
            // only one.
