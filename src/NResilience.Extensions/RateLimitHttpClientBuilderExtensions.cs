...
public static class RateLimitHttpClientBuilderExtensions
{
    ...
    public static IHttpClientBuilder AddRateLimit(this IHttpClientBuilder builder, RateLimiter limiter, string? name = null)
    {
        ...
        return builder.AddHttpMessageHandler(() => new RateLimitHandler(limiter, reported, false));
    }
    ...
    ///     The limit. Set exactly one of <see cref="RateLimitOptions.PermitsPerSecond" />, <see cref="RateLimitOptions.Permits" /> with
    ///     <see cref="RateLimitOptions.Window" />, or <see cref="RateLimitOptions.Concurrency" />.
    /// </param>
    /// <returns>The client builder.</returns>
    ...
    private static IHttpClientBuilder Add(IHttpClientBuilder builder, RateLimitOptions options)
    {
        ...
        if (!options.PerHost)
            return builder.AddHttpMessageHandler(() => new RateLimitHandler(options.ToLimiter(), reported, true));
