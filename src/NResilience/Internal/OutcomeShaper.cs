...
internal static class FailureException
{
    public static Exception Build(StopReason reason, Exception? error, TimeSpan deadline, AttemptLog attempts, TimeSpan? retryAfter)
    {
        ...
        // refused - and says it differently for each reason that reaches here, because one stopped
        // after a single attempt, one after all of them, and one because the process is going away.
        //
        ...
        var refusal = reason switch
        {
            ...
            StopReason.Draining =>
                "The call was rejected: this process is draining, so the result the policy refused was not retried.",
            _ =>
