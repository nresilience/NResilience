...
- **Declared members with no behavior**: `StreamResumed` and `Draining`, and the `Draining` stop reason, describe checkpointed stream resume and drain-aware shutdown. No execution path raises them: a retry over an `IAsyncEnumerable<T>` [stops at the first element](../faq.md#can-i-retry-a-stream), and the library reads no host lifetime. They are declared because the numbering of these enums is a contract for any listener that persists or exports a value, and a member added later renumbers everything after it. A `switch` over the kinds handles them the way it handles a kind it does not recognize.
...
IDs 1031 and 1032 belong to the two declared event kinds nothing raises, and are reserved beside them for the reason the members are: an ID is a contract the moment an alert is built on it.
