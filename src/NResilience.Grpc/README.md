...
That is the whole setup. The client now retries what is worth retrying, stops when the deadline
says so, opens a circuit per service, and tells the server how long each attempt has.
...
- Every gRPC call is an HTTP `POST`, which the HTTP handler refuses to retry by default, so the
  handler is inert.
...
`AddGrpcResilience()` is the gRPC-shaped registration, and the distinct name keeps the two from
